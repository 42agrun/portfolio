...
/*	This function is used at the start of minishell, it creates a custo-
	...
	Mode 0: used at the start of minishell for an exact copy of envp.
	Mode 1: used for the standalone "export" cmd, adds quotation marks.	*/
...
char	**copy_env(char **envp, int i, int mode)
{
	...
	while (envp[i])
	{
		if (mode == 0)
			...
			new[i] = add_new_export_str(envp[i], 0, 0, 0);
		if (!new[i])
		{
			write (2, "Error\nMalloc failed.\n", 21);
			ft_free_array(new);

...
/*	Print the dollar-sign if you encounter a non-letter or -number
	...
	Check echo_5 for the "display_known_flags" function!			*/
...
static void	handle_dollar_sign(char *echo, int *i, int *q_flag, char **envp)
{
	...
	if (!q_flag[0] && echo[i[0]] == '$')
	{
		if (echo[i[0] + 1] == '(' || echo[i[0] + 1] == '?')
			...
			special = check_alpha_num(echo, i, q_flag);
	}
	if (display_known_flags(echo, i, special, envp))
		return ;
		...
		&& echo[i[0]] != 9 && echo[i[0]] != '"')
	{
		if (special == 1 || special == 4)
			check_printout(echo[i[0]], NULL, 0, 0);
		i[0]++;
	}
...
static void	handle_double_quotes(char *str, int *i, int *quote_flag)
{
	...
	while (str[i[0]] && str[i[0]] != ' '
		&& str[i[0]] != '"' && str[i[0]] != '$')
	{
		check_printout(str[i[0]], NULL, 0, 0);
		i[0]++;
	}
}
...
/*	Using a while-loop in case multiple single quotation marks are next
	...
	Might need to revisit this function, final if-condition (39) is wonky.	*/

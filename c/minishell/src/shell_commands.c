...
/*	Using the $? commands returns the latest received exit value.
	...
	The standalone version of "export" (with no further arguments) simply
	prints out an env-list with "declare -x" in front of each line.		*/
...
static int	more_shell_commands(char *str, char **envp, int hide_error, int i)
{
	...
	else if (check_standalone_export(str))
		print_export_list(envp, hide_error);
	if (ft_strncmp("export", str, 0, 0) || ft_strncmp("unset", str, 0, 0))
		i = 1;
...
/*	Checking for commands which usually are followed by specific input.
	...
	Look up utils_6 for "check_correct_input".				*/
...
int	check_extended_shell(char *ref, char *s, int mode)
{
	...
	if (check_correct_input(s, ref, 0, 0))
...
static void	define_format_2(char *str, int i, int j)
{
	...
	else
	{
		...
		while (str[++i])
			buffer[++j] = str[i];
		buffer[j + 1] = 0;
		if (!check_extended_shell(buffer, NULL, 1))
			e_print(buffer, 0, 0, 5);
	}
}
...
void	define_format(char *str, char **env, int i, int j)
{
	...
	{
		getcwd(buffer, sizeof(buffer));
		e_print(buffer, 0, 0, 4);
	}
	else if (check_standalone_export(str))
		e_print("minishell: declare: `-x': not a valid identifier\n", 0, 0, 0);
	else
